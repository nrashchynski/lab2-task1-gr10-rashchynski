...
void remove_student_with_min_average(STUDENT students[], int *size) {
		...
		printf("Удалён студент: %s (Группа: %d, Средний балл: %.2f)\n",
					 students[min_index].name, students[min_index].group, students[min_index].average);
...
void input_buyers(BUYER buyers[], int size) {
		for (int i = 0; i < size; i++) {
				...
				scanf("%d %d %d", &buyers[i].birthdate.year, &buyers[i].birthdate.month, &buyers[i].birthdate.day);
				printf("Номер телефона: ");
				scanf("%s", buyers[i].phone);
				printf("Адрес (почтовый индекс страна область район город улица дом квартира): ");
				scanf("%d %s %s %s %s %s %d %d",
							&buyers[i].address.postal_code, buyers[i].address.country, buyers[i].address.region,
							buyers[i].address.district, buyers[i].address.city, buyers[i].address.street,
		...
		for (int i = 0; i < size; i++) {
				if (strcmp(buyers[i].address.city, "Брест") == 0) {
						printf("%s %s %s, Телефон: %s, Адрес: %s, %s, %s, %s, %s, %d, %d\n",
									 ...
									 buyers[i].phone, buyers[i].address.country, buyers[i].address.region,
									 buyers[i].address.district, buyers[i].address.city, buyers[i].address.street,
									 buyers[i].address.house, buyers[i].address.apartment);
						found = 1;
				}
...
void find_lucky_numbers() {
		...
		printf("Найдено %d счастливых номеров.\n", count);
}
...
int main(int argc, char *argv[]) {
		...
		if (size <= 0 || size > NUM_STUDENTS) {
				printf("Некорректное количество студентов. Допустимое значение: от 1 до %d.\n", NUM_STUDENTS);
				return 1;
		}
